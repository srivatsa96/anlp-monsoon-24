...
EN_SENT_ENC  = tokeniser([EN_SENT], return_tensors='pt', padding=True, truncation=True,max_length=100)['input_ids'].to(device)
FR_SENT = start_token
FR_SENT_ENC = tokeniser([FR_SENT], return_tensors='pt', padding=True, truncation=True,max_length=50)['input_ids'].to(device)
generated = tokeniser.decode(trained_model.generate(EN_SENT_ENC, FR_SENT_ENC, 100)[0].tolist(), skip_special_tokens=False)
print(generated.split('<|I_am_end|>')[0])
